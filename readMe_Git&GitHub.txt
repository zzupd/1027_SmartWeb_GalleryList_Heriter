...
5. Visual Studio Code 에서 Git을 사용한 
 GitHub에 연동하고 업로드하기
 1) 처음 GitHub에 업로드할 때의 순서
   (1) [소스제어] 아이콘 클릭 
   (2) [리포지터리 초기화] 버튼 클릭
   (3) 상단의 [소스제어] 메뉴의 목록(...) 클릭
   (4) [변경사항] => [모든 변경 내용 스테이징] 클릭
   (5) 상단의 [소스제어] 메뉴의 목록(...)
   (6) [분기] => [분기 이름 바꾸기] 클릭 후 
        master에서 main으로 변경
   (7) 상단의 [소스제어] 메뉴의 목록(...) 클릭
   (8) [커밋] => [스테이징된 항목 커밋] 클릭 후
        메시지 입력란에 저장소 이름 입력
        => 1027_SmartWeb_GalleryList_Heriter
             참고. 정상실행되면 모든 항목들이 사라짐
   (9) 상단의 [소스제어] 메뉴의 목록(...) 클릭
   (10) [원격] => [원격 추가] 클릭 후
         GitHub에서 생성한 리포지터리의 https 주소를 복붙
        => 참고. 훈련교사 해당 주소
https://github.com/zzupd/1027_SmartWeb_GalleryList_Heriter.git
        원격 이름에는 GitHub 저장소 이름 복붙

   (11) 상단의 [소스제어] 메뉴의 목록(...) 클릭
   (12) [풀, 푸시] => [다음으로 푸시...] 클릭 후
         해당 원격 깃허브 https 주소를 클릭

   (13) 토큰 인증과 관련된 팝업창이 나타나면
         팝업창에서 [token] 항목을 찾아 클릭하고
         보관해둔 토큰번호를 복붙한다.
         이후 만일 추가 인증을 요청할 경우
         [Device 로 인증] 또는 
         [username과 useremail 로 인증] 중 1가지 방식으로
         (사용자 이름, 사용자 이메일)
          추가 인증을 요청할 수 있음.

         관련정보를 입력창에 넣은 후 인증 진행하면
         1회 인증으로 추후 인증을 확인하지 않고
         업로드 가능함.



 2) 재업로드할 때의 순서
   (1) 코드 또는 폴더 및 파일 내용을 변경하고 저장
        => [소스 제어] 항목에서 변경사항이 적용된
             파일들이 등록됨
   (2)상단의 [소스제어] 메뉴의 목록(...) 클릭
   (3) [변경사항] => [모든 변경 내용 스테이징] 클릭
   (4) 상단의 [소스제어] 메뉴의 목록(...) 클릭
   (5) [커밋] => [스테이징된 항목 커밋] 클릭 후
        메시지 입력란에 수정된 내용에 관한 메시지 입력
   (6) 상단의 [소스제어] 메뉴의 목록(...) 클릭
   (7) [풀, 푸시] => [다음으로 푸시...] 클릭 후
         해당 원격 깃허브 https 주소를 클릭
